 ...
 */
__declspec(dllexport) disk::disk(string plugin_id, map<string,string> properties) 
{
    // Customize if needed
 ...
 */
__declspec(dllexport) disk::~disk(void)
{
    // Customize if needed
    sigar_close(sig_);
 ...
 * @param properties Map of properties (will be empty in this plugin).
 */
__declspec(dllexport) bool disk::initialize_plugin(map<string,string> properties) 
{
    // Customize if needed
    sigar_open(&sig_);
 ...
 * 
 * Gests the filesystems of a machine using Hyperic Sigar and publishes their status
 * using the method <code>publish_information</code>--defined and 
 ...
 */
__declspec(dllexport) bool disk::generate_and_publish_information(DDSDynamicDataWriter *writer,
