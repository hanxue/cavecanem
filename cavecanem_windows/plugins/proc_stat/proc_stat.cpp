 ...
 */
__declspec(dllexport) proc_stat::proc_stat(string plugin_id,
 ...
 */
__declspec(dllexport) proc_stat::~proc_stat(void)
{
    // Customize if needed
    sigar_close(sig_);
 ...
 * @param properties Map of properties (will be empty in this plugin).
 */
__declspec(dllexport) bool proc_stat::initialize_plugin(map<string,string> properties) 
{
    // Customize if needed
    sigar_open(&sig_);
 ...
 */
__declspec(dllexport) bool proc_stat::generate_and_publish_information(DDSDynamicDataWriter *writer,
