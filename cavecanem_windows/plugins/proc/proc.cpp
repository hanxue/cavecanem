 ...
 */
__declspec(dllexport) proc::proc(string plugin_id,
 ...
 */
__declspec(dllexport) proc::~proc()
{
    // Customize if needed
    sigar_close(sig_);
}
 ...
 * @param properties Map of properties (will be empty in this plugin).
 */
__declspec(dllexport) bool proc::initialize_plugin(map<string,string> properties) 
{
    // Customize if needed
    sigar_open(&sig_);
 ...
 */
__declspec(dllexport) bool proc::generate_and_publish_information(DDSDynamicDataWriter *writer,
