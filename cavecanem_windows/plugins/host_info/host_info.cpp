 ...
 */
__declspec(dllexport) host_info::host_info(string plugin_id,
 ...
 */
__declspec(dllexport) host_info::~host_info(void)
{
    // Customize if needed
    sigar_close(sig_);
 ...
 * @param properties Map of properties (will be empty in this plugin).
 */
__declspec(dllexport) bool host_info::initialize_plugin(map<string,string> properties) 
{
    // Customize if needed
    sigar_open(&sig_);
 ...
 */
__declspec(dllexport) bool host_info::generate_and_publish_information(DDSDynamicDataWriter *writer,
